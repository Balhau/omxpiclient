/*************************************************
 * BaLHau LIB
 * Esta biblioteca foi desenvolvida por Vítor Hugo Fernandes, a.k.a BaLHau
 * @author Balhau
 *
 * Método que devolve os elementos que contém uma determinada className
 * 
 * @param cls
...
/**
 * Função que nos devolve o elemento a partir do seu identificador, ou então os
...
/**
 * Objecto que contém os scripts da aplicação
 * 
 ...
 *                   objectos com propósitos mais específicos. Este consiste,
 *                   portanto, num contentor de bibliotecas. Poucos métodos são
 *                   directamente implementados neste objecto e em versões
 *                   futuras poderão mesmo ser reagrupados noutras estruturas
 * @author Balhau
 * @constructor Construtor da API
 */
var BLauLib = function() {
	this.versao = 0.1;
	this.author = "Ví­tor Fernandes, a.k.a Balhau";
};

/**
 * MÃ©todo que devolve uma string com as horas a partir do total de segundos
 * 
 * @param {Number}
 *            totseg
 */
...
/**
 * Método que devolve os valores representados numa string URL no formato GET
 * 
...
/**
 * Objecto com um conjunto de funções utilitárias para manipulação de cookies
 * 
 * @classDescription Objecto com um conjunto de funções utilitárias para
...
/**
 * Método que adiciona um novo cookie no browser
 * 
 * @param {String}
 *            name Nome do cookie
 * @param {String}
...
/**
 * Método que nos devolve o tempo restante de sessão, redireccinando no final
 * 
 * @param {String}
 *            divid Identificador do div que contem o tempo
 * @param {Number}
 *            totseg Número representando o total de segundos da hora.
 * @param {String}
 *            Link com o endereço da página no final da sessÃ£o
 */
...
/**
 * Libraria para invocação de páginas através do protocolo XMLHttpRequest
 * 
...
/**
 * Objecto que representa o estado da ligação
 * 
 * @classDescription Este objecto representa o estado da ligação assíncrona
 *                   efectuada
 */
BLauLib.Ajax.Status = {};
/**
...
BLauLib.Meses = [ "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro",
		"Dezembro" ];
...
BLauLib.Ajax.prototype._init_ = function() {
	...
	try {
		// tentativa para criar o objecto ajax para versÃµes nÃ£o IE
		...
		var XmlHttpVersions = [ 'MSXML2.XMLHTTP.6.0', 'MSXML2.XMLHTTP.5.0',
				...
				'Microsoft.XMLHTTP' ];
		// tenta todas as versÃƒÂµes
		for ( var i = 0; i < XmlHttpVersions.length && !xmlHttp; i++) {
			try {
				// tenta o objecto para as vÃ¡rias versÃµes do IE
...
/**
 * Adiciona um novo par (variável,valor) para envio no pedido ajax
 * 
 * @param {String}
...
/**
 * Método que devolve o objecto resposta do servidor enviado em formato JSON
 * 
...
/**
 * Limpa as variáveis POST presentes no objecto
 * 
 * @return {void}
 */
...
/**
 * Método que codifica o array de variáveis post para uma string com a sua
 * representação
 * 
 * @return{String} String com a representação das variáveis
 */
...
/**
 * Método que efectua uma chamada Assíncrona pelo protocolo POST
 * 
 * @param {String}
 *            pag Endereço da página
 */
...
BLauLib.Ajax.prototype.get=function(pag){
	this.open("GET",pag);
	this.send(null);
};
...
/**
 * Objecto para manipulação de chamadas assíncronas com recurso a iframes
 */
BLauLib.AIM = {};

/**
 * Método que cria um frame para chamada assí­ncrona
 * 
 * @param {Object}
 *            Object com as funções callback
 */
...
/**
 * MÃ©todo que efectua uma ligação assíncrona ao servidor
 * 
 * @param {String}
 *            metodo Tipo de ligação POST/GET
 * @param {String}
 *            urlAja Endereço da ligação
 */
BLauLib.Ajax.prototype.open = function(metodo, urlAja) {
	// Copia das estruturas devido Ã Â nÃ£o visibilidade do operador "this"
	// dentro do mÃ©todo "onreadystatechange"
 ...
 * @param ano
 *            Ano
 * @return Índice do dia da semana
 */
...
Date.DB_FORMAT=1;
/**
 * Devolve a data em vários formatos
...
/**
 * Método que devolve o número de dias de um mês
 * 
 * @param mes
 ...
 * de construção do elemento <b>object</b>
 */
...
/**
 * Responsável por devolver o elemento embed
 * @return Devolve o elemento <embed>
 */
...
/**
 * @return Devolve o elemento html.
 */
...
/**
 * Adiciona um parâmetro ao objecto SWF
 */
...
/**
 * Objecto que representa um objecto calendário
 * @classDescription Clase que implementa a funcionalidade de calendário
 */
...
/**
 * Método que efectua a criação da tabela contentora das datas
 * 
 * @param {obj}
 *            Elemento associado ao calendário
 */
BLauLib.UI.DatePicker.prototype._createTableDate = function(id) {
	var DS = 7;
	var id = id;
	var tbody = $$('table'); // Criação do elemento table que servirá de
	// contentor do calendário
	this.linhas = $$('tr', DS + 2); // Numero de linhas da tabela
	this.cells = $$('td', 49); // Numero de células 49, inclui dias do mes,
	// semana, cabeçalho e rodapé
	...
	var dias = this.cells.select(DS + 6, DS + 6 + 33);
	// Configuração dos dias do mes
	for ( var i = 3; i < DS + 1; i++) {
		this.linhas[i].add(dias.select(7 * (i - 3), 7 * (i - 2) - 1));
	}

	// Configuração dos botões de fundo
	...
	this.cells[47].colSpan = 7;
	this.cells[47].add(this.cancel);
	this.linhas[DS + 1].add(this.cells[47]);
...
BLauLib.UI.HourPicker.prototype.build = function() {
	...
	var min = dt.getMinutes();
	var seg = dt.getSeconds();
	for ( var i = 23; i >= 0; i--) { // Constrói a interface das horas;
		auxa = $$('option');
		v = i < 10 ? "0" + i : i;
		auxa.value = v;
		this.hsel.addOption(v, v, "opth_" + this.id);
	}
	for ( var i = 59; i >= 0; i--) { // Constrói a interface dos minutos e
										// segundos
		auxa = $$('option');
		auxb = $$('option');
		v = i < 10 ? "0" + i : i;
...
/**
 * Método privado ao objecto que pretende efectuar as células dos dias em função
 ...
 * @param {Array}
 *            Array com os cabeçalhos
 * @param {Array}
 *            Array com os dados tabelados
 * @param {Number}
 *            Number com o número de elementos por página
 * @param {String}
...
/**
 * Devolve o número de páginas necessárias para a representação dos dados
 */
...
/**
 * Método responsável por mudar o conteúdo de página
 * 
 * @param npage
 *            Numero da página
 * @return Nada
...
/**
 * Função que remove uma entrada no objecto NPaged
 * 
 * @param n
 *            Índice da entrada
 * @param callback
...
BLauLib.UI.NPaged.prototype.removeEntry = function(n, callback, s) {
	...
	if (n < 0 || n >= this.data.length) // caso o valor de n esteja fora do
		// intervalo de dados
		return;
	if (n >= posmin && n < posmax) {// caso o valor n pertenca à página que se
...
BLauLib.UI.NPaged.prototype.build = function() {
	...
	this.dfoot = $$('div');// divisor com os nÃºmeros
	var aux1;
	var aux2;
...
/**
 * Método que adiciona os dados ao objecto
 * @param dt Informação com os dados de procura
 * @return
 */
...
/**
 * Classe para efectuar a concatenação de strings de uma forma eficiente,
 ...
 * 
 * @param {String}
 *            str String que se pretende concatenar com a já existente
...
/**
 * Método que devolve o objecto string representado
 * 
 * @return {String} String resultante da concatenação
 */
...
/**
 * Remove os elementos a partir do valor de endereço do objecto
 * 
 * @param {Object}
...
/**
 * Método que remove os elementos "undefined" e null de um array
 * 
 * @return {Array} Array resultante da remoção de elementos null e undefined
 */
...
/**
 * Efectua a remoção de caracteres vazios à esquerda da string
 * 
 * @return {String} String sem espaços brancos à esquerda
 */
...
/**
 * Efectua a remoção de caracteres vazios à direita da string
 * 
 * @return {String} String sem espaços brancos à direita
 */
...
/**
 * Método que devolve um array de elementos que devolvem valor true quando
 * passados para a função callback
 * 
 * @param {Function}
 *            callback Função callback que avalia o elemento
 * @return {Array} Array com os elementos que retornam valor true na avaliação
 *         pela função callback
 */
...
/**
 * Método que aplica uma função a todos os elementos de um determinado array
 * 
 * @param callback
 *            Função callback executada sobre os elementos
 * @return true ou false dependendo se o argumento da função é uma função ou
 *         não.
 */
...
/**
 * Devolve uma string com a representação dos elementos contidos no Array
 * 
 * @return {String} String com a representação dos elementos presentes no Array
 */
...
/**
 * Método que efectua a remoção do último elemento do array
 * 
 * @return {Object} Elemento presente no array
 */
Array.prototype.popv = function() {
...
/**
 * Devolve a posição que um determinado objecto ocupa no array
 * 
 * @param {Object}
 *            valor Objecto do array
 * @return {Number} Posição do elemento do array, -1 caso o elemento não
 *         pertença ao array
 */
...
/**
 * Método que adiciona um elemento ao array
 * 
 * @param {Object}
 *            Objecto, ou lista de objectos a adicionar
 * @return {Array} devolve o próprio array para efeitos de chaining
 */
...
/**
 * Método que remove um elemento do array
 * 
 * @param {Number}
 *            indice Posição do elemento a remover
 * @return {Array} devolve o array já sem o elemento
 */
...
/**
 * Devolve a divisão inteira entre dois números arredondada por defeito
 * 
 * @return {Number} Valor da divisão
 */
...
/**
 * Devolve a divisão inteira entre dois números arredondada por excesso
 * 
 * @return {Number} Valor da divisão
 */
...
/**
 * Valida uma string a partir de uma expressÃ£o regular
 * 
 * @return {Boolean} Verdadeiro caso a string verifique a condição dada pela
 *         expressão regular, falso caso contrário
 */
...
/**
 * Método que extende a classe String, adicionando a possiblidade de reverter a
...
/**
 * Método que transforma uma String num array com a sua codificação UNicode
 * 
 * @param {String}
 *            car Caracter separador
 * @return {String} Codificação Unicode separada através do caracter car
 */
 ...
 *            sup Valor máximo
 * @param {bool}
 *            intarray Valor booleano indicando se o array é composto por
...
/**
 * Método que devolve um array bidimensional com valores aleatórios
 * 
 * @param {Number}
...
/**
 * Objecto reservado para implementação de funções para animar objectos
 */
HElement.FX = {};

/**
 * @param {Object} obj Contem os detalhes sobre a animação. O objecto deve ter a seguinte estrutura
...
HElement.FX.animate=function(obj){
	...
	if(!Array.equals(iparams,eparams))
		return -1;
	var callAnim=function(){//metodo que efectua a animação
		if(step>totsteps){
...
/**
 * Método que especifica a transparência do objecto em HTML
 * 
 * @param {Object}
...
/**
 * Método que devolve a posição de um determinado elemento
 * 
...
/**
 * Método que atribui uma posição a um elemento
 * 
 * @param {Number}
 *            x Valor de X
 * @param {Number}
...
/**
 * Método que devolve a actual transparência aplicada ao objecto
 * 
 * @return Number Percentagem de transparência
 */
...
/**
 * Método que remove um elemento do DOM
 * 
 * @return
 */
...
/**
 * Método que adiciona valores quando estamos na presença de um objecto do tipo
 * select. Este método foi concebido devido às diferenças existentes entre os
 * vários browsers
 * 
 * @param {Object}
 ...
 */
// MÃƒÂ©todo para adicionar valores a elementos
...
/**
 * Método que devolve a opção actualmente seleccionada para um elemento do tipo
 * SELECT
 * 
 * @return {@link Option}
 */
HElement.prototype.getSelectedOption = function() {
	if (this.nodeName != "SELECT") {
		throw "Este método só está disponível para elementos do tipo SELECT";
	}
	return this.options[this.selectedIndex];
};

/**
 * Método que aplica um efeito de desvanecimento a elementos de HTML
 * 
 * @param {Number}
 *            seg Número de segundos ate atingir a transparencia mínima
 * @see #getAlpha
 * @see #setAlpha
 * @see #aparece
 */
...
/**
 * Método que aplica um efeito de reaparecimento a elementos de HTML
 * 
 * @param {Number}
 *            seg Número de segundos ate atingir a transparencia máxima
 * @see #setAlpha
 * @see #setAlpha
 * @see #desvanece
 */
...
/**
 * Método que efectua a duplicação em profundidade de um determinado Elemento
 * HTML. Quando invocado este método cria um elemento idêntico ao elemento
 * instanciado assim como os Elementos filhos num processo recursivo. Os
 * identificadores são diferentes de cópia para cópia para evitar colisões em
 * posteriores invocações de objectos a partir de javascript partindo do seu ID;
 * 
 * @param {Booleano}
 *            Variável boleana indicando o tipo de clonagem. Caso este parãmetro
 *            não seja inserido ou seja verdadeiro a cópia é feita em
 *            profundidade, caso o valor seja falso temos unicament a cópia do
...
/**
 * Método que coloca um elemento no centro da janela
 */
...
/**
 * Método que aplica uma função de callback aos elementos em profundidade
 * 
 * @param {Function}
 *            callback
 */
...
/**
 * Método que efectua o resize de um objecto
 * 
 * @param {Event}
 *            mev Evento de rato
 * @param {Number}
 ...
 * @param {Number}
 *            miny Altura mínima
 * @deprecated Método não deve ser utilizado, efectuar o resize utilizando a
 *             especificação CSS3
 */
 ...
 * 
 * @classDescription Este objecto representa um contentor janela. O propósito
 *                   consiste em simular um ambiente desktop a partir de
 *                   construção de um conjunto de elementos e alguns métodos
 *                   associados
 * @param {String}
...
/**
 * Método que devolve o div contentor da janela
 */
...
/**
 * Método que atribui uma determinada transparência à janela
 * 
 * @param {Number}
 *            val Transparência, valor entre 0 e 100
 */
...
/**
 * Método que permite especificar o conteúdo do contentor da janela
 * 
 * @param {String}
 *            val String contendo o HTML que irá ficar interno ao contentor da
 *            janela
 */
...
/**
 * Método que permite especificar o conteúdo da barra de título da janela
 * 
 * @param {String}
 *            titulo HTML com o conteúdo a passar para a janela de título
 */
 ...
 *                   funcionalidades que irão operar sob a tag canvas. Este
 *                   objecto consiste numa pequena API para automatizar algumas
 *                   ferramentas e funcionalidades de computação visual que se
 *                   pode efectuar recorrendo à especificação HTML5
 * @param {String}
...
BLauLib.Canvas = function(id) {
	...
	if (this.canvas && this.canvas.getContext) {
		...
		throw "O browser não suporta o objecto canvas";
};
...
/**
 * Método que especifica a função que será executada quando se efectua um
 * varrimento da imagem pixel por pixel. A função enviada para o método será
 ...
 * @param {Object}
 *            args Argumentos adicionais para a função callback
 */
...
/**
 * Método que efectua uma inversão de cor
 */
...
/**
 * Converte uma string para uma string de representação hexadecimal
 * 
 * @return {String} String com a representação hexadecimal
 */
...
/**
 * Método que transforma uma string de codigos hexadecimais na sua representação
 * de caracteres
 * 
...
/**
 * Converte um número Decimal para a sua representação Hexadecimal
 * 
 * @return {Number} representação hexadecimal
 */
...
/**
 * Converte um número Hexadecimal para a sua representação Decimal
 * 
 * @return {Number} representação Decimal
 */
 ...
 * 
 * @param {Number}
 *            alpha parametro de transparência
 */
...
/**
 * Método que carrega uma imagem para o objecto canvas efectuando um
 * redimensionamento a partir do seu comprimento máximo
 * 
 * @param {String}
 *            src Endereço da imagem
 * @param {Number}
 *            xmax Comprimento máximo da imagem
 */
...
/**
 * Método que carrega uma imagem para o objecto canvas efectuando um
 * redimensionamento a partir da sua altura máxima
 * 
 * @param {String}
 *            src Endereço da imagem
 * @param {Number}
 *            xmax Altura máxima da imagem
 */
...
/**
 * Método que efectua a transicao entre dois niveis de transparencia
 * 
 * @param {Number}
...
/**
 * Método que remove um elemento filho a partir do seu identificador
 * 
 * @param {String}
...
/**
 * Método que devolve o atributo do elemento a partir do seu identificador
 * 
 * @param {String}
...
/**
 * Método que especifica valores a atributos
 * 
 * @param {String}
...
/**
 * Método que adiciona um novo elemento filho ou um conjunto de elementos
 * 
 * @param {HElement}
 *            arg Elemento filho
 */
...
/**
 * Método que efectua a conversao de um determinado elemento para a estrutura
 * HElement
 * 
 * @param {Element}
...
HElement.get = function(ele) {
	...
	for (metodo in interf) {
		// so assina se nÃ£o estiver definido
...
/**
 * Método que desabilita um elemento
 * 
 * @param {Boolean}
 *            val True para desabilitar, false para habilitar
 * @param {Boolean}
 *            recurs Recursivo ou não recursivo
 */
...
/**
 * Método que especifica o valor do campo innerText (wrapper)
...
/**
 * Método que especifica o valor do campo innerHTML (wrapper)
...
/**
 * Devolve o número de filhos de um HElement
 * 
 * @return {Number} Número de filhos de um HElement
 */
...
/**
 * Método que remove elementos filhos a partir da sua posição ou valor
 * 
 * @param {Number,Object}
 *            val
 */
...
/**
 * Objecto com um conjunto de funcionalidades para gestão de eventos
 * 
 ...
 *                   aplicações foi criado este objecto para encapsular a
 *                   funcionalidades referidas. Aqui encontramos um mecanismo de
 *                   lista. Para cada evento existe uma lista com os métodos a
 *                   invocar quando este é accionado. Desta forma pode-se ter
 *                   mais do que uma função associada aos vários eventos
 *                   disponíveis
 */
BLauLib.Events = {
	loadwindow : [],
 ...
 */
BLauLib.Temp={};
BLauLib.Temp.onload=window.onload;
...
/**
 * Objecto que contem um conjunto de metodos de validação de expressões
 * regulares para as principais entradas de formulário
 */
BLauLib.Valida = {
...
/**
 * Método que converte uma string json para o seu correspondente objecto
 */
...
/**
 * Método que converte um objecto para a sua representação JSON
 * 
 * @return {String} Representação JSON do objecto
 */
...
/**
 * Método que codifica uma string para uma outra que consiste na sequencia de
 * numeros representando o caracter sob a numeração ascii separados por um
 * caracter especial
 * 
 * @param {Object}
 *            caracterseparador Caracter de separação
 */
...
/**
 * Método que transforma uma string codificada contendo os codigos unicode da
 * string separados por um caracter separador. para o seu valor descodificado
 * 
 * @param {String}
 *            caracter separador
 * @return {String} String correspondente Ã dada pela codificação unicode
 */
...
/**
 * Método que codifica uma string em formato UTF-8
 * 
...
/**
 * Método que descodifica uma string do formato UTF-8
 * 
...
/**
 * Método que contém um conjunto de funcionalidades para o processamento de
 * captchas
 * 
 * @classDescription Este objecto pretende fornecer um conjunto de
 *                   funcionalidades de manipulação de imagens gerados pelo
 *                   sistema RECaptcha
 * @param {String}
...
/**
 * Método que utiliza um script de servidor para guardar imagens geradas pelo
 * serviço RECaptcha
 * 
 * @param {String}
...
/**
 * Devolve um array com o valor percentual ocupado por cada valor na respectiva posição
 */
...
/**
 * Nome do ficheiro que funciona como proxy para endereços externos.
 */
RECaptcha.PHPRedirectFile = "redirect.php";

/**
 * Endereço do serviço RECaptcha
 */
RECaptcha.ServerName = "http://www.google.com/recaptcha/api/";

/**
 * Método que efectua a herança para um objecto
